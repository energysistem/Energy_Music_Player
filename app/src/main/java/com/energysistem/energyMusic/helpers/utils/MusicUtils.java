...
import com.energysistem.energyMusic.service.ServiceBinder;
...
import static com.energysistem.energyMusic.Constants.GENRES_DB;
import static com.energysistem.energyMusic.Constants.PLAYLIST_NAME_FAVORITES;
import static com.energysistem.energyMusic.Constants.PLAYLIST_NEW;
import static com.energysistem.energyMusic.Constants.PLAYLIST_QUEUE;
...
public class MusicUtils {
    ...
    public static long[] getSongListForFolder(Context context, String path) {
        ...
        selection.append(AudioColumns.IS_MUSIC + "=1");
        selection.append(
                " AND " + MediaColumns.TITLE + " != '' AND "+ Audio.Media.DATA+" like '"+path+"%'");
        String mSortOrder = MediaColumns.TITLE;
        Uri uri = Audio.Media.getContentUriForPath(path);
        Cursor cursor = context.getContentResolver().query(uri, projection, selection.toString(),
                null, mSortOrder);
        if (cursor != null) {
            long[] list = getSongListForCursor(cursor);
    ...
    public static long createPlaylist(Context context, String name) {
        ...
        if (name != null && name.length() > 0) {
            ...
            Cursor cur = resolver.query(Playlists.EXTERNAL_CONTENT_URI, cols, whereclause,
                    null, null);
            if (cur.getCount() <= 0) {
    ...
    public static void renamePlaylist(Context mContext, long id, String name) {
        ...
        if (name != null && name.length() > 0) {
            ...
            ContentValues values = new ContentValues(1);
            values.put(PlaylistsColumns.NAME, name);
            resolver.update(Playlists.EXTERNAL_CONTENT_URI, values, BaseColumns._ID + "=?",
                    new String[] {
                            String.valueOf(id)
                    });
    ...
    public static void makePlaylistList(Context mContext, boolean create_shortcut,
        ...
        if (resolver == null) {
            ...
            where.append(Playlists.NAME + " != ''");
            where.append(" AND " + Playlists.NAME + " != '" + PLAYLIST_NAME_FAVORITES + "'");
            Cursor cur = resolver.query(Playlists.EXTERNAL_CONTENT_URI, cols,
                    where.toString(), null, Playlists.NAME);
            list.clear();
